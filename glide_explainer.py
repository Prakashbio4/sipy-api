...
def _coerce_ctx(ctx: Dict[str, Any]) -> Dict[str, Any]:
    """
    Ensures context is a dictionary.
    """
...
def explain_glide_story(ctx: Dict[str, Any]) -> Dict[str, Any]:
    """
    Returns a layman, ≤3-sentence story about the glide path.
    """
    ctx = _coerce_ctx(ctx)
    ...
    # 2. Describe the mid-journey transition
    mid_sentence = "As you move closer to your goal, the portfolio will automatically de-risk, stepping down to a more balanced mix of **~{}% equity** midway through the journey.".format(mid_eq)
